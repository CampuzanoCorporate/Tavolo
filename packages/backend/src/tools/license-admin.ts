...
import { generateLicense, refreshLicense, updateLicenseStatus } from '../modules/licensing/licensing.service';
...
function readIntArg(flag: string) {
...
async function renewExistingLicense() {
  ...
  const validityDays = readIntArg('--days');
  const license = await refreshLicense(id, validityDays);
  console.log(`Licencia renovada: ${license.code}`);
  console.log(`Nueva validez: ${license.validUntil.toISOString()}`);
}

async function changeStatus(status: 'SUSPENDED' | 'CANCELLED') {
  const id = readIntArg('--id');
  if (!id) {
    throw new Error('Debes indicar --id <licenseId>');
  }

  const license = await updateLicenseStatus(id, status);
...
async function activateByCode() {
  ...
  const license = await prisma.license.findUnique({ where: { code: code.trim().toUpperCase() } });
  if (!license) {
