...
async function main() {
  ...
  // ── 3. Usuarios ───────────────────────────────────────────────────────────
  const pwAdmin   = await bcrypt.hash('Admin1234!', 12);
  const pwCarlos  = await bcrypt.hash('Camarero1!', 12);
  const pwCocina  = await bcrypt.hash('Cocina111!', 12);
  ...
  async function seedVenueCatalog(venue: typeof venueCentro, label: string) {
    const ensureCategory = async (name: string, color: string, sortOrder: number) => {
    ...
    for (const product of productosData) {
      ...
      if (!existing) {
        await prisma.product.create({
          data: product as Parameters<typeof prisma.product.create>[0]['data'],
        });
      }
    }
